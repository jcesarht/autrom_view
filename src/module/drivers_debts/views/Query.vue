        ...
        <div v-if="tableData && tableData.length > 0" class="overflow-x-auto">
            <table class="min-w-full bg-white border border-gray-200 rounded-lg">
                ...
                <tbody class="divide-y divide-gray-200">
                    <tr v-for="row in tableData" :key="row.dribeb_id">
                        <td class="px-4 py-2 text-sm">{{ row.dribeb_sequence }}</td>
                        <td class="px-4 py-2 text-sm">{{ row.dri }}</td>
                        <td class="px-4 py-2 text-sm">{{ row.drideb_amount }}</td>
                        <td class="px-4 py-2 text-sm">{{ row.drideb_type == '1' ? 'Tarifa' : 'Otros' }}</td>
                        <td class="px-4 py-2 text-sm">{{ row.drideb_date }}</td>
                        <td class="px-4 py-2 text-sm">{{ row.drideb_details }}</td>
                    </tr>
                </tbody>
            </table>
        </div>
...
<script setup>
    ...
    const { query, data, error, message } = useDriversDebts()
