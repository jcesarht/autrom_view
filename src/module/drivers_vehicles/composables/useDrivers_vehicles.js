import { drivers_vehiclesService } from "../services/drivers_vehiclesService"; 
...
export function useDrivers_vehicles(){
    ...
    const save = async ( param_data ) => {
        try{
            ...
            const res = await drivers_vehiclesService.save( param_data, {"token":token} )
            if (res.error){
                throw Error(res.result.message)
            }
            
            message.value = "Record "+res.result.data.data.driveh_id+" Saved succesfully."
    ...
    const query = async (filter = undefined)=>{
        try{
            ...
            message.value = '';
            const {token} = useUserLoginStore()
            const res = await drivers_vehiclesService.query( filter, {"token":token} )
            if (res.error){
                throw Error(res.result.message)
            }
            ...
            for (let i = 0; i < result_data.length; i++) {
                if (result_data[i].driveh_id) {
                    result_data[i].actions_buttons = useActionsTableRecord(result_data[i].driveh_id);
                }
                ...
                dataForUpdate[result_data[i].driveh_id] = result_data[i];
            }
    ...
    const update = async (id, param_data) => {
        try{
            ...
            message.value = '';
            const {token} = useUserLoginStore()
            const res = await drivers_vehiclesService.update( id, param_data, {"token":token} )
            if (res.error){
                throw Error(res.result.message)
            }
    ...
    const remove = async (id) => {
        try{
            loading.value = true;
            const {token} = useUserLoginStore()
            const res = await drivers_vehiclesService.remove( id, {"token":token} )
            if (res.error){
                throw Error(res.result.message)
            }
            if (res.error === false) {
