import { providesService } from "../services/providesService"; 
...
export function useProvides(){
    ...
    const save = async ( param_data ) => {
        try{
            ...
            const res = await providesService.save( param_data, {"token":token} )
            if (res.error){
                throw Error(res.result.message)
            }
            
            message.value = "Record "+res.result.data.data.pro_id+" Saved succesfully."
    ...
    const query = async (filter = undefined)=>{
        try{
            ...
            message.value = '';
            const {token} = useUserLoginStore()
            const res = await providesService.query( filter, {"token":token} )
            if (res.error){
                throw Error(res.result.message)
            }
            ...
            for (let i = 0; i < result_data.length; i++) {
                if (result_data[i].pro_id) {
                    result_data[i].actions_buttons = useActionsTableRecord(result_data[i].pro_id);
                }
                ...
                dataForUpdate[result_data[i].pro_id] = result_data[i];
            }
    ...
    const update = async (id, param_data) => {
        try{
            ...
            message.value = '';
            const {token} = useUserLoginStore()
            const res = await providesService.update( id, param_data, {"token":token} )
            if (res.error){
                throw Error(res.result.message)
            }
    ...
    const remove = async (id) => {
        try{
            loading.value = true;
            const {token} = useUserLoginStore()
            const res = await providesService.remove( id, {"token":token} )
            if (res.error){
                throw Error(res.result.message)
            }
            if (res.error === false) {
