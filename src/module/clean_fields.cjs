...
files.forEach(file => {
    ...
    const removeRegex = /<div>\s*<(UIInputText|UIAutocomplete|UIInputDate)[\s\S]*?name=\"(com|[a-zA-Z0-9_]+_(create_at|update_at))\"[\s\S]*?(?:<\/\1>|\/>)\s*<\/div>/g;
    content = content.replace(removeRegex, '');

    // In Register.vue, remove sub completely
    if (file.endsWith('Register.vue')) {
        const removeSubRegex = /<div>\s*<(UIInputText|UIAutocomplete|UIInputDate)[\s\S]*?name=\"sub\"[\s\S]*?(?:<\/\1>|\/>)\s*<\/div>/g;
