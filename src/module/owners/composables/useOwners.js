...
export function useOwners(){
    ...
    const save = async ( param_data ) => {
        try{
            ...
            const res = await ownersService.save( param_data, {"token":token} )
            if (res.error){
                throw Error(res.result.message)
            }
            
            message.value = "Record "+res.result.data.data.own_id+" Saved succesfully."
    ...
    const query = async (filter = undefined)=>{
        try{
            ...
            message.value = '';
            const {token} = useUserLoginStore()
            const res = await ownersService.query( filter, {"token":token} )
            if (res.error){
                throw Error(res.result.message)
            }
            ...
            for (let i = 0; i < result_data.length; i++) {
                if (result_data[i].own_id) {
                    result_data[i].actions_buttons = useActionsTableRecord(result_data[i].own_id);
                }
                ...
                dataForUpdate[result_data[i].own_id] = result_data[i];
            }
    ...
    const update = async (id, param_data) => {
        try{
            ...
            message.value = '';
            const {token} = useUserLoginStore()
            const res = await ownersService.update( id, param_data, {"token":token} )
            if (res.error){
                throw Error(res.result.message)
            }
    ...
    const remove = async (id) => {
        try{
            loading.value = true;
            const {token} = useUserLoginStore()
            const res = await ownersService.remove( id, {"token":token} )
            if (res.error){
                throw Error(res.result.message)
            }
            if (res.error === false) {
