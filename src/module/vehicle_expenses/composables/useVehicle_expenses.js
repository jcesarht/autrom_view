import { vehicle_expensesService } from "../services/vehicle_expensesService"; 
...
export function useVehicle_expenses(){
    ...
    const save = async ( param_data ) => {
        try{
            ...
            const res = await vehicle_expensesService.save( param_data, {"token":token} )
            if (res.error){
                throw Error(res.result.message)
            }
            
            message.value = "Record "+res.result.data.data.vehexp_id+" Saved succesfully."
    ...
    const query = async (filter = undefined)=>{
        try{
            ...
            message.value = '';
            const {token} = useUserLoginStore()
            const res = await vehicle_expensesService.query( filter, {"token":token} )
            if (res.error){
                throw Error(res.result.message)
            }
            ...
            for (let i = 0; i < result_data.length; i++) {
                if (result_data[i].vehexp_id) {
                    result_data[i].actions_buttons = useActionsTableRecord(result_data[i].vehexp_id);
                }
                ...
                dataForUpdate[result_data[i].vehexp_id] = result_data[i];
            }
    ...
    const update = async (id, param_data) => {
        try{
            ...
            message.value = '';
            const {token} = useUserLoginStore()
            const res = await vehicle_expensesService.update( id, param_data, {"token":token} )
            if (res.error){
                throw Error(res.result.message)
            }
    ...
    const remove = async (id) => {
        try{
            loading.value = true;
            const {token} = useUserLoginStore()
            const res = await vehicle_expensesService.remove( id, {"token":token} )
            if (res.error){
                throw Error(res.result.message)
            }
            if (res.error === false) {
