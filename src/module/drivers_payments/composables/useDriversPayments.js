...
import { driversPaymentsService } from '../services/driversPaymentsService'

export const useDriversPayments = () => {
    const data = ref(null)
    ...
    const save = async (formData) => {
        ...
        try {
            const response = await driversPaymentsService.save(formData, { token })
            if (response.error) {
                error.value = true
    ...
    const update = async (id, formData) => {
        ...
        const { token } = useUserLoginStore()
        try {
            const response = await driversPaymentsService.update(id, formData, { token })
            if (response.error) {
                error.value = true
    ...
    const query = async (params = null) => {
        ...
        const { token } = useUserLoginStore()
        try {
            const response = await driversPaymentsService.query(params, { token })
            if (response.error) {
                error.value = true
    ...
    const remove = async (id) => {
        ...
        const { token } = useUserLoginStore()
        try {
            const response = await driversPaymentsService.remove(id, { token })
            if (response.error) {
                error.value = true
