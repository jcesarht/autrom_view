...
export function useShift_Settlement(){
    ...
    const save = async ( param_data ) => {
        try{
            ...
            const res = await shift_settlementService.save( param_data, {"token":token} )
            if (res.error){
                throw Error(res.result.message)
            }
            
            message.value = "Record "+res.result.data.data.shiset_id+" Saved succesfully."
    ...
    const query = async (filter = undefined)=>{
        try{
            ...
            message.value = '';
            const {token} = useUserLoginStore()
            const res = await shift_settlementService.query( filter, {"token":token} )
            if (res.error){
                throw Error(res.result.message)
            }
    ...
    const update = async (id, param_data) => {
        try{
            ...
            message.value = '';
            const {token} = useUserLoginStore()
            const res = await shift_settlementService.update( id, param_data, {"token":token} )
            if (res.error){
                throw Error(res.result.message)
            }
    ...
    const remove = async (id) => {
        try{
            loading.value = true;
            const {token} = useUserLoginStore()
            const res = await shift_settlementService.remove( id, {"token":token} )
            if (res.error){
                throw Error(res.result.message)
            }
            if (res.error === false) {
